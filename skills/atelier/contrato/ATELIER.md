...
1. **DESCOBRE** — filtre `docs/indices/skills_index.json` (do agent-skills-hub; na Gaia, sem esse índice, vá direto ao IMPROVISA) pela palavra-chave da peça (mesmo filtro determinista do ORQUESTRACAO: targets+risk, depois category/ranking lexical). Critério de match bom: a skill cobre a peça *inteira* na especialidade dela (ex.: peça é "empty state" → `ux-feedback`). Match bom → convoque a skill como artesão.
